// src/services/api.js
import { avisarFinDeSesion, borrarSesionLocal, leerToken } from "./sesion";
...
/* === Sesión terminada (401) ===
   ...
   Dentro del panel ya no se redirige: `avisarFinDeSesion` pinta el aviso
   encima de la pantalla (AvisoSesion) y lo escrito sigue ahí hasta volver a
   entrar. Fuera del panel se mantiene la vuelta a la portada con su aviso. */
async function handleUnauthorized(r) {
  if (!leerToken()) return false; // no había sesión que cerrar
  const cuerpo = r ? await r.clone().json().catch(() => ({})) : {};
  borrarSesionLocal();
  ...
  // en la portada. Y se deja una marca para explicárselo.
  try {
...
async function makeRequest(method, url, body, extraHeaders = {}) {
  ...
  // Si había sesión, `handleUnauthorized` ya la cerró y avisó; no hay nada
  // más que hacer. Si no la había (página pública), se sigue el flujo normal para que
  // la vista pueda mostrar el mensaje real que devolvió el servidor.
  if (r.status === 401 && await handleUnauthorized(r)) return {};
